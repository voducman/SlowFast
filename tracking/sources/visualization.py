...
class Visualization():
    ...
    def __init__(self, seq_info, output_video_file=None):
        image_shape = seq_info["image_size"][::-1]
        aspect_ratio = float(image_shape[1]) / image_shape[0]
        image_shape = 1024, int(aspect_ratio * 1024)
        self.viewer = ImageViewer(image_shape, "Figure %s" % seq_info['video_name'])
        self.viewer.thickness = 2
        self.frame_idx = seq_info["min_frame_idx"]
        self.last_idx = seq_info["max_frame_idx"]
        if output_video_file is not None:
            self.viewer.enable_videowriter(output_video_file, fps=24, fourcc_string='XVID')
    ...
    def process(self, frame, detections, tracks, frame_idx):
        self.set_image(frame)
        self.draw_detections(detections)
        self.draw_trackers(tracks)
        self.viewer.process(frame_idx)
